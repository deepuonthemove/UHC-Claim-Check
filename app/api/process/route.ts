  ...
  const stream = new ReadableStream({
    async start(controller) {
      ...
      try {
        ...
        // 3. Parse claim rows — always sent as JSON from the client
        const claimRowsJson = formData.get('claimRows') as string | null;
        if (!claimRowsJson) {
          await sendEvent({ type: 'error', message: 'Missing claimRows JSON in request.' });
          return;
        }
        ...
        // 4. Read login credentials from uploaded Excel
        const loginExcelFile = formData.get('loginExcel') as File | null;
        if (!loginExcelFile) {
          await sendEvent({ type: 'error', message: 'Missing loginExcel file.' });
          return;
        }
